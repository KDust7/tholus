...
The `javascript` job installs the bun workspace, then:
...
The `wasm engine` job installs the Rust toolchain pinned by `rust-toolchain.toml` plus a
...
`bash scripts/verify-local.sh` runs the parts of the `javascript` job that fail on drift instead of
on behavior: build, lint, typecheck, the emitted protocol schemas, release readiness, the public
API report, and the fork-rewrite tests. It reports every failure; it does not stop at the first.
...
Exceeding the raw or brotli budget prints a warning explaining the consequence, but does not fail
the build. Past roughly 20 MB of wasm, V8 stops caching compiled code and every visit pays a full
recompile.
...
artifact that job just built. That is the point: a parity run against a stale artifact proves
nothing.
...
- Read the tarballs back. `scripts/check-tarballs.mjs` opens each `.tgz`, parses the tar in
  process, and inspects the `package.json` npm wrote into it. It refuses on a surviving
